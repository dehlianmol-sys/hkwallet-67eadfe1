...
create table if not exists public.agents (
  ...
  commission_percentage numeric(6,2) not null default 5,
...
select public.hk_add_column('agents','commission_percentage', 'numeric(6,2) not null default 5');
...
create table if not exists public.agent_commissions (
  ...
  rate           numeric(6,2) not null default 5,
...
select public.hk_add_column('agent_commissions','rate',           'numeric(6,2) not null default 5');
...
as $$
  with rate as (
    select coalesce(max(commission_percentage), 5) as pct
